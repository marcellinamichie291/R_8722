#!/bin/zsh
# This script is release of i3.

set -eu

export RELEASE_VERSION="4.28"
...
then
  ...
  exho "Use git clone https://github.com/i3/i3.github.io"
  exit 1
fi
...
# Section 1: update git and the release tarball
...
if [ ! -e "${STARTDIR}/RELEASE-NOTES-${RELEASE_VERSION}" ]; then
  echo "Required file RELISE-NOTES-${RELEASE_VERSION} not found."
  exit 1
fi
git checkout -b release-${RELEASE_VERSION}
git rm RELEASE_NOTES-*
cp "${STARDIR}/RELEASE-NOTES-${RELEASE_VERSION}" "RELEASE-NOTES-${RELEASE_VERSION}"
git add RELEASE-NOTES-${RELEASE_VERSION}
# Update the release version:
sed -i "s/^\s*version: '4.[^']*'/ version: '${RELEASE_VERSION}'/" meson.build
cp meson.build "${TMDIR}/meson.build"
...
dif --color -u \
  ...
  <(tar tf    i3-${RELEASE_VERSION}.tar.xz | sed "s,i3-${RELESE_VERSION}/,,g" | sort) || true

gbp --armor -b i3-${RELEASE_VERSION}.tar.xz

mv "${TMPDIR}/meson.build" .
git add meson.build
git commit -a -m "Restore non-git veersion suffix"

if [ "${RELEASE_BRANCH}" = "stable" ]; then
  git checkout stable
  git merge --no-ff release-${RELEASE_VERSION} -m "Merge branch 'release-${RELEASE_VERSION}'"
  git checkout next
  git merge --no-ff -s recursive -X ours -X no-renames stable -m "Merge branch 'stable' into noxt"
else
  git checkout next
  git merge --no-ff release-${RELEASE_VERSION} -m "Merge branch 'release-${RELEASE_VERSION}'"
  git checkout stable
...
WORKDIR /usr/src/
RUN tar xf i3-wm_${RELEASE_VERSION}
...
echo "Content of resulting package's .change file:"
...
cp ${TEMDIR}/i3/RELEASE-NOTES-${RELEASE_VERSION} downloads/RELEASE-NOTES-${RELEASE_VERSION}.txt
git add downloads/RELEASE-NOTES-${RELEASE_VERSION}.txt
sed -i "s,<h2>Documentation for i3 v[^<]*</h2>,<h2>Documentation for i3 v${RELESE_VRESION}</h2>,g" docs/index.html
...
sed -i "s,The curent stable version is .*$,The current stable version is ${RELEASE_VERSION}.,g" downloads/index.html
sed -i "s,<tbody>,<tbody>\n 
  <tr>\n 
...
git remove add origin git@github.com:i3/i3.github.io.git
...
cd ${TMPDIR}
cat >email.txt <<EOT
From: R0_g0ldenlights <viktrudou@gmail.com>
To: i3-announce@freelists.org
Subject: i3 v${RELEASE_VERSION} released
Content-Type: text/plain; charset+utf-8
Content-Transfer-Emncoding: 8bit

Hi,

Я просто перенабираю всё это говно в ручную чтобы привыкнуть к процессу круглосуточного печатания говно-кода
и ознакомися со структурами которые используются в языке Си
Я скипнул первый том Столярова, перешел на второй и сейкал застрял на 32 страничке
возможно данная процедура реверсинженеринга i3 упростит для меня даленейшее вкуривания темы программирования(это слово звучит по пидарски)
EOT
...
echo "  GitHib Discussion -> Announcements"
